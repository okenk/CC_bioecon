...
For the root-finding method, probability of fishing in a given week is the inverse-logit of scaled expected profit (revenue for the week - variable costs). For example, if expected profit is zero, there is a 50% chance of fishing. Expected profit must be scaled otherwise the probability will always hover in the vicinity of 0.5. The scaling parameter was chosen so that there is a 95% chance of fishing in a week when revenue is 105% of variable costs. The root-finding method does not involve any random decision-making. Instead, ships are ordered from most to least likely to fish (constant through time), the probability of fishing is multiplied by the number of ships and rounded the nearest whole number $n$, and the most eager $n$ ships fish in that week.

Fishers were not assumed to interfere with one another and catch rates within a week are independent of the numbers of vessels fishing. For crab, the available population decreases each week based on previous catches. For salmon, it is assumed new fish are continually migrating into the fishing area throughout the season, and the modeled recruitment is more of a relative index of catch rates for the entire season. Catchability, which I define as the proportion of the available stock that is caught by one "ship" in one week, is modeled as constant in time. (This means catch rates for crab go down, but stay constant for salmon.)

Problem: When I solve for variable costs, only 66% of crab recruits get caught in the average year. Seems like we want to get closer to 100%. I am using a selectivity so that 16% of the stock is caught in the first week. Playing around with the selectivity and the logistic scaling parameter didn't do much. Or is there an assumption off somewhere? 

An aside on the salmon: If the same number of salmon are available each week, the number of salmon that migrate in each week is inversely related to how many were caught.
