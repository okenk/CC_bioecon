...
Cost per trip is scaled so that in the average year, costs = revenue. However, calculating the variable cost such that, on average, annual costs cover annual revenue cannot be done explicitly. I used univariate root-solving to find the cost per trip that allows revenue to cover costs (fixed + variable) in an average year. For simplicity (for now?), this was done independently for the two fisheries, and did not account for the fact that dual permit ships could switch to the other fishery. 

For the root-finding method, probability of fishing in a given week is the inverse-logit of scaled expected profit (revenue for the week - variable costs). For example, if expected profit is zero, there is a 50% chance of fishing. Expected profit must be scaled otherwise the probability will always hover in the vicinity of 0.5. The scaling parameter was chosen so that there is a 95% chance of fishing in a week when revenue is 125% of variable costs. The root-finding method does not involve any random decision-making. Instead, ships are ordered from most to least likely to fish (constant through time), the probability of fishing is multiplied by the number of ships and rounded the nearest whole number $n$, and the most eager $n$ ships fish in that week.

Fishers were not assumed to interfere with one another and catch rates within a week are independent of the numbers of vessels fishing. For crab, the available population decreases each week based on previous catches. For salmon, it is assumed new fish are continually migrating into and out of the fishing area throughout the season, and the modeled recruitment is more of a relative index of catch rates for the entire season. Catchability, which I define as the proportion of the available stock that is caught by one "ship" in one week, is modeled as constant. (This means catch rates for crab go down, but stay constant for salmon.) Right now, catchability for crab assumes `r catchability['crab'] * sum(ships_per_fleet[c('crab', 'both')]) * 100`% of crab recruits are caught in the first week.

Problem: I am having trouble tuning the model. By following those methods, only 78% of crab recruits are caught in an average year. This seems low. This also means variable costs are much lower than anticipated, so fixed costs make up 38% of average revenue instead of 30%. I can either increase fixed costs and get catches closer to 100% of recruitment, or decrease fixed costs and get fixed costs closer to 30% of realized average revenue. Changing catchability doesn't seem to do much to this issue, though the logistic scaling parameter does. The more slowly fishers change their minds on whether or not it's worth it to fish, the greater the total catches. I think this is because some people stay in the fishery longer even when they're fishing at a loss. I feel like I'm sort of getting stuck at this very elementary step. Though maybe it's less elementary than I think because I'm building the mechanics of the model.
